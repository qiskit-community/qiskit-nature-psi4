...
lint:
	pylint -rn qiskit_nature_psi4 test tools
	python tools/verify_headers.py qiskit_nature_psi4 test tools
	python tools/find_stray_release_notes.py

mypy:
	mypy qiskit_nature_psi4 test tools

style:
	black --check qiskit_nature_psi4 test tools docs

black:
	black qiskit_nature_psi4 test tools docs
...
spell:
	pylint -rn --disable=all --enable=spelling --spelling-dict=en_US --spelling-private-dict-file=.pylintdict qiskit_nature_psi4 test tools
...
coverage:
	coverage3 run --source qiskit_nature_psi4 -m unittest discover -s test -q
	coverage3 report
